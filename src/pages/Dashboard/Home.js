...
} from '../../store/profiles/profileActions';
...
const Home = () => {
	...
	const { email, token } = useSelector(state => state.auth.userInfo);
    ...
    if (userProfile) {
      const _id = userProfile._id;
		dispatch(getMutualInterestProfiles({ _id, token }));
    }
	}, [dispatch, userProfile,token]);
    ...
    if (userProfile) {
      const _id = userProfile._id;
		dispatch(getSentInterestProfiles({ _id, token }));
    }
	}, [dispatch, userProfile, token]);
    ...
    if (userProfile) {
      const _id = userProfile._id;
		dispatch(getReceivedInterestProfiles({ _id, token }));
    }
	}, [dispatch, userProfile, token]);
	...
	const {
		freshProfiles,
		sentInterestProfiles,
		receivedInterestProfiles,
		mutualInterestProfiles } = useSelector(state => state.profiles);
				...
				<div className="row">
						...
						<div className="card gradient-1 card-bx">
							<div className="card-body d-flex align-items-center">
								<div className="me-auto text-white">
									<h2 className="text-white">{(freshProfiles && freshProfiles.length)||0}</h2>
									<span className="fs-18">Fresh Profiles</span>
								</div>
					...
					<div className="col-xl-3 col-sm-6">
						<div className="card gradient-2 card-bx">
							<div className="card-body d-flex align-items-center">
								<div className="me-auto text-white">
									<h2 className="text-white">{ (mutualInterestProfiles&&mutualInterestProfiles.length)||0}</h2>
									<span className="fs-18"> Mutual Interests </span>
								</div>
								<svg width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<div className="col-xl-3 col-sm-6">
						<div className="card gradient-3 card-bx">
							<div className="card-body d-flex align-items-center">
								<div className="me-auto text-white">
									<h2 className="text-white">{(sentInterestProfiles && sentInterestProfiles.length)||0}</h2>
									<span className="fs-18">Interests Sent</span>
								</div>
								<svg width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<div className="col-xl-3 col-sm-6">
						<div className="card gradient-4 card-bx">
							<div className="card-body d-flex align-items-center">
								<div className="me-auto text-white">
									<h2 className="text-white">{(receivedInterestProfiles && receivedInterestProfiles.length)||0}</h2>
									<span className="fs-18">Interests Received</span>
								</div>
								...
								<div className="card text-center">
									<div className="card-body">
										<div id="radialChart" className="radialChart">
											{<RadialDonut profileCompletion={userProfile ? userProfile.profileCompletion : "0"} />}
										</div>
										<h2> {userProfile ? userProfile.profileCompletion : "0"} %</h2>
										<span className="fs-16 text-black">Profile Completion</span>
										<div style={{ marginTop: "20px" }}>
											{!userProfile ? <div><h4 className="text-danger">Your Profile is empty</h4><Link to="/form-add-basic-profile" > Click Here to Fill up Your Profile </Link> </div> : <div></div>}
						...
						<div className="card">
							...
							<div className="card-body">
								<div style={{ marginTop: "0px" }}>
									{
										userProfile ? // check for user profile
											userProfile.accountStatus ? // check if account is activated 
												<div>
													<h2 className='text-success'>Your account is active</h2>
												</div>
												:
											
												<div>
													<h2 className="text-primary">Your account is inactive</h2><Link to="/activate-account" > Click here to activate your account </Link>
												</div>
											: 
											<div>
												<h2 className="text-info">You Need A Profile for Your Account</h2>
