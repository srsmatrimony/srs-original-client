...
import { getSentInterestProfiles } from '../../../../store/profiles/profileActions';
...
const ProfilesInterstSent = () => {
  ...
  const { sentInterestProfiles, loading ,error} = useSelector(state => state.profiles);
  ...
  if (loading) {
    ...
    return (
      <Fragment>
        <PageTitle activeMenu="Profiles in which Interest is Sent" motherMenu="Profiles" />
        {
          userProfile.accountStatus ? // check if account is activated
          <div className="row">
            {sentInterestProfiles ? sentInterestProfiles.map((profile) => <SingleProfileList key={profile._id} profile={profile} />) :
          
              <div className="text-danger text-center ">
                No profiles found. please sent interest to profiles which you like .
              </div>
            }
...
export default  ProfilesInterstSent ;
