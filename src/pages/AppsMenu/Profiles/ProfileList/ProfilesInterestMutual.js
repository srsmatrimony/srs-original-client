...
import { getMutualInterestProfiles } from '../../../../store/profiles/profileActions';
...
const ProfilesInterestMutual = () => {
  ...
  const { mutualInterestProfiles,loading,error } = useSelector(state => state.profiles);
  ...
  if (loading) {
    ...
    return (
      <Fragment>
        <PageTitle activeMenu="Mutually Interested Profiles" motherMenu="Profiles" />
        {
          userProfile.accountStatus ? // check if account is activated

          <div className="row">
            {mutualInterestProfiles ? mutualInterestProfiles.map((profile) => <SingleProfileList key={profile._id} profile={profile} />) :
          
              <div className="text-danger text-center ">
                No profiles found. please wait a while for a profile to show up for mutual interest.
              </div>
