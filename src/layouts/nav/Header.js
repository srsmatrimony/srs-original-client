...
const Header = (props) => {
	...
	const [userType, setuserType] = useState('Basic User');
  ...
  var finalName = filterName.includes("app")
	...
	const handleSearch = () => {
		...
		if (!searchWord) {
			swal("error","please enter content for search");
			return 
		}
		navigate(`/profiles-search?search=${searchWord}`);
		setsearchWord('');
            ...
            <ul className="navbar-nav header-right main-notification">
				<li className="nav-item">
					<div className="input-group search-area">
						<input type="text" className="form-control" placeholder="Search For Profiles By First Name" value={searchWord} onChange={(e) => setsearchWord(e.target.value)} onKeyPress={handleKeyPress}/>		  
								  <span className="input-group-text" style={{ cursor: 'pointer' }} onClick={handleSearch}><i className="flaticon-381-search-2"></i></span>		  
					</div>		  
				</li>
				...
				<Dropdown as="li" className="nav-item dropdown header-profile">
					<Dropdown.Toggle variant="" as="a" className="nav-link i-false c-pointer">
						...
						<div className="header-info ms-3">
							<span>{userEmail||"user@mail.com"}</span>
					...
					<Dropdown.Menu align="right" className="mt-3 dropdown-menu dropdown-menu-end">
					  <Link to="/app-profile" className="dropdown-item ai-icon">
						<svg
						  id="icon-user1" xmlns="http://www.w3.org/2000/svg" className="text-success"
					  ...
					  {/* <Link to="/email-inbox" className="dropdown-item ai-icon">
						...
						<span className="ms-2">Inbox </span>
								  ...
								  <AccountDeactivate className='text-danger' />
								  <LogoutPage />
